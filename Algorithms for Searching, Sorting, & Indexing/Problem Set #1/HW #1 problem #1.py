...
def findCrossoverIndexHelper(x, y, left, right):
    ...
    if left >= right:
        print("Element not in x\n")
        return False
    else:
        mid = (left + right) //2

    print(f"    Mid:{mid} | x[mid]: {x[mid]}, y[mid]: {y[mid]}, x[mid + 1]: {x[mid +1 ]}, y[mid + 1]: {y[mid + 1]}")
    
    if x[mid] > y[mid]:
        if x[mid + 1] < y[mid + 1]:
            print(f"Solved. i={mid}")
            return mid
        elif x[mid + 1] > y[mid + 1]:
            return findCrossoverIndexHelper(x, y, left=(mid + 1), right=right)    
    else: # x[mid] < y[mid]
        if x[mid + 1] < y[mid + 1]:
            return findCrossoverIndexHelper(x, y, left=left, right=(mid - 1))
        else:   # x[mid + 1] > y[mid + 1]
            return findCrossoverIndexHelper(x, y, left=(mid + 1), right=right)
